...
<nav class="navbar navbar-expand-lg navbar-transparent  bg-primary  navbar-absolute">
  ...
  <div class="content" style="background-image: url('{{ asset('assets/img/hora.jpg') }}'); background-repeat: none; background-size: cover;">
   <div class="row">
      <div class="col-md-2"></div>
      <div class="col-md-8">
        <div class="card">
          <div class="card-header">
            <h5 class="title"><i class="far fa-clock"></i>
              {{__(" Crear nuevo horario")}}
            </h5>
          </div>
          <div class="card-body">
            <form method="post" action="{{ route('horarios.store') }}" autocomplete="off"
                ...
                <div class="row">
                  <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="time" name="inicioH" class="form-control" required >
                    </div>
                  </div>
                  <div class="col-md-6">
                    <div class="form-group">
                        <label>{{__("Hora de fin")}}</label>
                        <input type="time" name="finH" class="form-control" required >
                    </div>
                  </div>
     ...
     <div class="row">
      ...
      <div class="col-md-10">
        <div class="card">
          <div class="card-header">
            <h5 class="title"><i class="now-ui-icons tech_watch-time"></i>
            {{__("Horarios registrados")}}
            </h5>
          </div>
          <div class="card-body">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td style="text-align:center;">
                      <form class="eliminar" method="post" action="{{ route('horarios.destroy', $a->id_horario) }}">
                        @csrf
                       @method('delete')
                       <a onclick="cargardatos('{{$a->id_horario}}','{{$a->horario_inicio}}','{{$a->horario_fin}}')"data-toggle="modal" data-target="#exampleModal" class="btn btn-success btn-icon btn-sm " >
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form method="post" action="{{ route('horarios.update',0) }}">
         @csrf
         @method('put')
        ...
        <div class="row">
          ...
          <div class="col-md-5">
            <div class="form-group">
                    <label>{{__("Hora de inicio")}}</label>
                     <input id="inicio" type="time" name="inicio" class="form-control" required >

                    </div>
                    <input id="idHorario" name="idHorario" type="hidden" >

          </div>
          <div class="col-md-5">
            <div class="form-group">
                      <label for="exampleInputEmail1">{{__("Hora de fin")}}</label>
                      <input id="fin" type="time" name="fin" class="form-control" required >
                    </div>
...
<!-- Alertas para el ingreso horario -->
@if(session('mensaje')=='repetido')
  <script type="text/javascript">
    Swal.fire(
      'Horario repetido',
      'El horario se encuentra repetido.',
      'warning'
      )
  </script>
@endif
@if(session('mensaje')=='guardados')
  <script type="text/javascript">
    Swal.fire(
      'Horario guardado',
      'El horario se ingreso correctamente.',
      'success'
      )
  </script>
@endif
@if(session('mensaje')=='eliminado')
  <script type="text/javascript">
    Swal.fire(
      'Horario elimanado',
      'El horario se elimino correctamente.',
      'success'
      )
  </script>
@endif
@if(session('mensaje')=='NoEli')
  <script type="text/javascript">
      Swal.fire(
        'Horario no eliminado',
        'El horario no se puede eliminar porque pertenece a encuentros.',
        'error'
      )
  </script>
@endif
@if(session('mensaje')=='editado')
  <script type="text/javascript">
    Swal.fire(
      'Horario editado',
      'El horario se edito correctamente.',
      'success'
      )
  </script>
@endif
  ...
  <script type="text/javascript">
    function cargardatos(id,Hinicio,Hfin){
      document.getElementById("inicio").value=Hinicio;
      document.getElementById("fin").value=Hfin;
      document.getElementById("idHorario").value=id;
    }
    $(document).ready(function(event) {
      $('.eliminar').submit(function(eve) {
        eve.preventDefault();
